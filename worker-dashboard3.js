...
applyButton.addEventListener('click',async()=>{if(applyButton.disabled)return;const text=overview.value.trim();if(!text){result.textContent='登録を希望する資料の概要を入力してください。';overview.focus();return}if(!confirm('この内容で抽選に申し込みますか？'))return;applyButton.disabled=true;result.textContent='申込みを受け付けています……';try{const r=await fetch('/lottery-apply',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({overview:text})}),d=await r.json().catch(()=>null);if(!r.ok||!d?.success){result.textContent=d?.message||'抽選申込みを受け付けられませんでした。';if(r.status!==503)applyButton.disabled=false;return}result.innerHTML='';const p=document.createElement('p');p.className='important';p.textContent='抽選申込みを受け付けました。次のAP番号を紙などに控えてください。';result.appendChild(p);const al=document.createElement('div');al.textContent='確認番号（AP番号）';result.appendChild(al);const ap=document.createElement('div');ap.className='code';ap.textContent=d.ap||'';result.appendChild(ap);const w=document.createElement('div');w.className='ap-privacy';w.textContent='AP番号は他人に教えないでください。';result.appendChild(w);const save=document.createElement('p');save.textContent='AP番号は、抽選結果の確認・登録申請・追加提出・登録書の受取りに使います。紙などに控え、大切に保管してください。';result.appendChild(save);const n=document.createElement('p');n.textContent='抽選結果は、文化資料登録室の「抽選結果確認」でAP番号を入力して確認してください。';result.appendChild(n)}catch(e){result.textContent='抽選申込みに失敗しました。しばらく時間をおいて再度お試しください。';applyButton.disabled=false}});loadStatus();
</script></body></html>`;
