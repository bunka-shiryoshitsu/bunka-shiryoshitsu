...
export async function adminKeyCacheStore(request,env,storage,now=Date.now()){
 ...
 try{
  if(body.action==='records'){
   ...
   if(misses.length){const fresh=await env.REGISTRATION_KV.get(misses);for(const name of misses){const value=fresh.get(name)??null;values[name]=value;if(value===null||typeof value==='string'&&new TextEncoder().encode(value).length<90000)await storage.put(recordPrefix+name,{value,until:now+(value===null?60000:RECORD_TTL)})}}
