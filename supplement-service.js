...
function publicRound(round,now){return {...round,status:currentStatus(round,now),deadlineLabel:deadlineLabel(round.deadline)};}
  ...
  try {
    ...
    if(path.endsWith('/status')) {
      ...
      for(const item of application.items||[]) {
        ...
        items.push({item:item.item,name:item.finalName||item.name||'',reviewResult:final?.result||item.reviewResult||'',registrationStatus:item.registrationStatus||'',rounds:rounds.map(r=>publicRound(r,now)),revision:record?.revision||0});
      }
    ...
    const finalized=Boolean(final)||Boolean(item.registrationNumber)||item.registrationStatus==='cancelled'||item.reviewResult==='rejected'||round?.status==='closed';
    ...
    await storage.put(key,record);
    return json({success:true,message:round.status==='submitted'?'追加資料を受け付けました。週に一度、この画面をご確認ください。':'保存しました。',round:publicRound(round,now)});
