...
const key=()=>window.AdminSession?window.AdminSession.credential():$('#adminKey').value.trim();const H=(json=true)=>{const h={'X-Admin-Key':key()};if(json)h['Content-Type']='application/json';return h};
...
async function api(url,opt={}){const r=await fetch(url,opt);const t=await r.text();let d=null;try{d=t?JSON.parse(t):null}catch{}if(!r.ok){const e=new Error(r.status===401?'管理キーを確認できませんでした。画面上部の管理キーを入力し直して「管理情報を表示」を押してください。':d?.message||t||('HTTP '+r.status));e.status=r.status;throw e}return d}
...
$('#adminKey').value=window.AdminSession?window.AdminSession.initialKey():sessionStorage.getItem('bunkaAdminKey')||'';$('#login').onclick=loadAll;$('#refresh').onclick=loadAll;$('#systemRefresh').onclick=loadAll;
...
async function loadItemImages(ap,item,container){
 ...
 try{
  ...
  for(let i=1;i<=20;i++){
   ...
   const blob=await r.blob();if(!container.isConnected)return;
   const src=URL.createObjectURL(blob),img=document.createElement('img');img.src=src;img.alt='資料 '+item+' の審査画像 '+no;
   img.onclick=()=>window.open(src,'_blank','noopener');container.appendChild(img);count++;
   const observer=new MutationObserver(()=>{if(!img.isConnected){URL.revokeObjectURL(src);observer.disconnect();}});observer.observe(document.getElementById('detail'),{childList:true,subtree:true});
  }
  notice.textContent=failed?'画像を読み込めませんでした。管理認証を確認して再度開いてください。':count?'画像を押すと拡大できます。':'保存されている審査画像はありません。';
 }catch{notice.textContent='画像を読み込めませんでした。もう一度開いてください。';}
}
