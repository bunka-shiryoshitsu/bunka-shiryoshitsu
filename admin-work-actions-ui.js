...
(() => {
 ...
 function decorate(){
  ...
  for(const row of document.querySelectorAll('#lotteryTable tbody tr')){const ap=row.cells?.[0]?.textContent.trim();if(ap)row.id='lottery-'+ap;row.classList.toggle('work-table-target',tasks.some(t=>t.kind==='lottery'&&t.ap===ap));}
